"""Connection lifecycle for Navan Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from navan_client import NavanClient
...
@chat.function(
    ...
    action_type="write",
    chain_callable=True,
    event="navan-connector.connect_navan",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_navan(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Execute connect_navan lifecycle handler."""
    client = NavanClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "Navan Account",
    ...
    return ActionResult.success(ConnectionRecord(id=cid, label=record["label"], masked_key=_mask(params.api_key), base_url=params.base_url, is_active=True), summary="Navan connected.")
...
@chat.function(
    ...
    action_type="write",
    chain_callable=True,
    event="navan-connector.disconnect_navan",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_navan(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    """Execute disconnect_navan lifecycle handler."""
    conns = await _load_connections(ctx)
    ...
    return ActionResult.success(DeleteResult(id=target["id"], deleted=True, message="Disconnected successfully"), summary="Navan disconnected.")
